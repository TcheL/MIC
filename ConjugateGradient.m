function [modelsltn, rsdl2norm] = ConjugateGradient(Gmatrix, data, ...
  epsr2norm, epsupdate)
%
% [model, rsdl2norm] = ConjugateGradient(Gmatrix, data, epsr2norm, epsupdate)
%
% This is a CG (Conjugate Gradient Method) program that solve the problem
% d = Gm, i.e. data = Gmatrix*model, that don't have to be a symmetric
% positive definite system of equations.
% Note that this program can similarly use CGLS (conjugate gradient least
% squares method) to solve the least squares problem
% min ||Gm - d||_2 for d = Gm.
%
% Written by Tche L. at USTC, 2015/12.
%
% modelsltn: a vector whose size is [paranum, 1],
%            the model solutions given by this program.
% rsdl2norm: a constant variable,
%            the 2-norm of the residual of the model modelsltn.
%
...
%
% Ref.: The Algorithm 6.4 on the P.154 and the Algorithm 6.5 on the P.155 of
...
while(1)
  ...
  if(norm(rknewresidual, 2) <= epsr2norm ...
    || mean(abs(modelupdate)) <= epsupdate)
    break;
  elseif(its >= itsmax)
    warning('MATLAB:MICMaximumIterationTime', ['The time of Conjugate ', ...
      'Gradient iterating comes to %d, and the Conjugate Gradient ', ...
      'iteration has been forced to exit, as a result, the solution ', ...
      'may be not enough accurate.'], its);
    break;
  end
  betastepfactor = (rknewresidual'*rknewresidual) ...
    /(rklastresidual'*rklastresidual);
...
%
% This is a program of judging a matrix whether it is a symmetric positive
% definite matrix.
% If A is a symmetric positive definite matrix, the function will return
% SpdIndex = 1; otherwise, SpdIndex = 0.
%
% Written by Tche L. at USTC, 2015/12.

% SpdIndex: a constant variable,
%           the symmetric-positive-definite index.

% A: a matrix,
%    the matrix we want to judge whether it is a symmetric positive
%    definite matrix.
