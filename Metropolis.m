function [msltn,rsdl2norm] = Metropolis(fwdfun,modelinit,Tempseries,pararange,obsvstn,obsvdat)
% [msltn,rsdl2norm] = Metropolis(fwdfun,modelinit,Tempseries,pararange,obsvstn,obsvdat)
% This is a Metropolis inversion program for the problem obsvdat = fwdfun(msltn,obsvstn) in the range restrain pararange of model parameters.
...
% modelinit: a vector whose size is [paranum,1], the initial model.
...
%
% Ref.: Metropolis Algorithm on the P.86 of the textbook Global Optimization Methods in Geophysical Inversion (Second Edition. Mrinal K. Sen, Paul L. Stoffa. 2012. Cambridge University Press.)
...
for i = 1:1:Tempnum
    for j = 1:1:randmovenum(i)
        while(1)
            ...
            if(sum(modelnew >= pararange(:,1)) + sum(modelnew <= pararange(:,2)) == 2*paranum)
                % if all model parameters of modelnew are not less than their avaliable minimum and not greater than their avaliable maximum.
                break;
            end
        end
