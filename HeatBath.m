function [msltn, rsdl2norm] = HeatBath(fwdfun, Tempseries, pararange, ...
  parasmplnum, obsvstn, obsvdat)
%
% [msltn, rsdl2norm] = HeatBath(fwdfun, Tempseries, pararange, parasmplnum, ...
%   obsvstn, obsvdat)
%
% This is a Heat Bath inversion program for the problem
...
%              the sample number of every model parameter in its range.
...
%
% Ref.: Heat Bath Algorithm on the P.92 of the textbook Global Optimization
...
for k = 1:1:paranum
  modelparapoint(k, :) = pararange(k, 1) ...
    + (0:1:parasmplnum(k) - 1).*parasmplintv(k);
end
...
for i = 1:1:Tempnum
  for j = 1:1:randmovenum(i)
    for k = 1:1:paranum
      ...
      if(Gibbsprobsum == 0)
        % If each errorenergy is very large, the sum of Gibbsprob is close
        % to 0, the latter procedure will go wrong.
        % Thus, if so, immediately jump out to do next loop.
        continue;
      end
