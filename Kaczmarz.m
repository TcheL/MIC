...
% This is a Kaczmarz's Algorithm program that solve the problem data = Gmatrix*model.
% Written by Tche.L. from USTC, 2015,12.
%
% model: a vector whose size is [paranum,1], the model parameter solutions given by this program.
% rsdl2norm: a constant variable, the 2-norm of the residual of the model.
%
...
%
% Ref.: The Algorithm 6.1 on the P.146 of the textbook Parameter Estimation And Inverse Problems (Second Edition. Richard C. Aster, Brian Borchers, Clifford H. Thurber. 2011. Academic Press.)
...
while(1)
    its = its + 1;
    for i = 1:1:datanum
        modelupdate(:,i) = (Gmatrix(i,:)*model - data(i))/(Gmatrix(i,:)*Gmatrix(i,:)')*Gmatrix(i,:)';
    ...
    if(rsdl2norm <= epsr2norm || sum(abs(modelupdate(:)))/paranum <= epsupdate)
        break;
    elseif(its >= itsmax)
        warning('MATLAB:MyMaximumIterationTime','The time of Kaczmarz''s iterating comes to %d, and the Kaczmarz''s iteration has been forced to exit, as a result, the solution may be not enough accurate.',its);
        break;
    end
end
