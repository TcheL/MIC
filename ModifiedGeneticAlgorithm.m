function [msltn,rsdl2norm] = ModifiedGeneticAlgorithm(fwdfun,pararange,paragenelen,obsvstn,obsvdat,fitnessmaxlimit,allowquitfitmax,allowquitfitavemaxratio)
% [msltn,rsdl2norm] = ModifiedGeneticAlgorithm(fwdfun,pararange,paragenelen,obsvstn,obsvdat,fitnessmaxlimit,allowquitfitmax,allowquitfitavemaxratio)
% This is a modified genetic algorithm inversion program for the problem obsvdat = fwdfun(msltn,obsvstn) in the range restrain pararange of model parameters.
...
%
% Ref.: Genetic Algorithm on the P.135 of the textbook Global Optimization Methods in Geophysical Inversion (Second Edition. Mrinal K. Sen, Paul L. Stoffa. 2012. Cambridge University Press.)
...
while(1)
    ...
    % Fitness evaluation
    fitnessnew = EvaluateFitness(fwdfun,modelpopunew, ...                     	% the fitness of every model from modelpopunew.
        obsvstn,obsvdat,fitnessmaxlimit);
    % Stopping criteria
...
function modelfitness = EvaluateFitness(fwdfun,modelset,obsvstn,obsvdat,fitnessmaxlimit)
% modelfitness = EvaluateFitness(fwdfun,modelset,obsvstn,obsvdat,fitnessmaxlimit)
% This is a program used to evaluate model fitness by a genetic algorithm program.
% Written by Tche.L. from USTC, 2015,12.

% modelfitness: a vector whose size is [1,modelnum], the fitnesses of these models from modelset.

% fwdfun: a function handle, the forward function of the inversion problem of the genetic algorithm.
% modelset: a matrix whose size is [paranum,modelnum], the model set whose fitness we want to evaluate.
% obsvstn: a vector whose size is [datanum,~], the observe station point x for the observe data.
% obsvdat: a vector whose size is [datanum,1], the observe data used to inverse the problem obsvdat = fwdfun(model,obsvstn).
% fitnessmaxlimit: a constant variable, the maximum value of fitness that can be tolerated.
