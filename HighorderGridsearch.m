function [modelsltn,rsdl2norm] = HighorderGridsearch(fwdfun,pararange,parasmplnum,obsvstn,obsvdat)
% [modelsltn,rsdl2norm] = HighorderGridsearch(fwdfun,pararange,parasmplnum,obsvstn,obsvdat)
% This is a high-order (multi-dimension) grid search inversion program for the problem obsvdat = fwdfun(modelsltn,obsvstn) in the range restrain pararange of model parameters.
% Note that this program can be used to a problem where the number of model parameters is arbitrary.
% Written by Tche.L. from USTC, 2016.1.
%
% modelsltn: a vector whose size is [paranum,1], the model solution given by this program.
% rsdl2norm: a constant variable, the 2-norm of the residual for the model modelsltn.
%
...
% parasmplnum: a vector whose size is [paranum,1], the number of the sampling points of every parameter.
...
% This is a calculate point position coordinate program called by the multi-dimesion grid search program HighorderGridsearch.
% Written by Tche.L. from USTC, 2016.1.

% pointposition: the point position coordinate calculated by this funtion.

% ordernum: a constant variable, the order number (sequence number) of the calculate point.
% paranum: a constant variable, the number of the model parameters, i.e. paranum = length(nodalpointnum).
% nodalpointnum: a vector whose size is [paranum,1], the number of nodal points in the difference dimesion space; but note that the ith element represents total nodal points of the (i - 1)-dimesion space.
