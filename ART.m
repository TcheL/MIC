...
% This is a ART (Algebraic Reconstruction Technique) program that solve the tomographic reconstruction problem traveltime = Gmatrix*gridmodel.
...
% epsupdate: a small positive constant variable, the maximum model update amount to allow stop iterating.
%
% Ref.: The Algorithm 6.2 on the P.147 of the textbook Parameter Estimation And Inverse Problems (Second Edition. Richard C. Aster, Brian Borchers, Clifford H. Thurber. 2011. Academic Press.)
...
while(1)
    ...
    for i = 1:1:tnum
        for j = 1:1:paranum
            ...
            modelupdate = (iprdctime - traveltime(i))*updatecoeffi(i,j)/raylen(i); % the model update amounts.
    ...
    if(rsdl2norm <= epsr2norm || maxmodelupdate <= epsupdate)
        break;
    elseif(its >= itsmax)
        warning('MATLAB:MyMaximumIterationTime','The time of ART iterating comes to %d, and the ART iteration has been forced to exit, as a result, the solution may be not enough accurate.',its);
        break;
    end
end
