function [gridmodel, rsdl2norm] = SIRT(Gmatrix, traveltime, epsr2norm, ...
  epsupdate)
%
% [gridmodel, rsdl2norm] = SIRT(Gmatrix, traveltime, epsr2norm, epsupdate)
%
% This is a SIRT (Simultaneous Iterative Reconstruction Technique) program that
% solve the tomographic reconstruction problem traveltime = Gmatrix*gridmodel.
%
...
%
% Ref.: The Algorithm 6.3 on the P.148 of the textbook Parameter Estimation
...
while(1)
  ...
  for i = 1:1:size(Gmatrix, 1)
    ...
    modelupdate = modelupdate + (traveltime(i)/raylen(i) ...
      - iprdctime/cellnum(i))*updatecoeffi(i, :)';
  end
  ...
  if(rsdl2norm <= epsr2norm || mean(abs(modelupdate)) <= epsupdate)
    break;
  elseif(its >= itsmax)
    warning('MATLAB:MICMaximumIterationTime', ['The time of SIRT ', ...
      'iterating comes to %d, and the SIRT iteration has been forced ', ...
