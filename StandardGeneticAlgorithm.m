function [msltn, rsdl2norm] = StandardGeneticAlgorithm(fwdfun, pararange, ...
  paragenelen, obsvstn, obsvdat, fitnessmaxlimit, allowquitfitmax, ...
  allowquitfitavemaxratio)
%
% [msltn, rsdl2norm] = StandardGeneticAlgorithm(fwdfun, pararange, ...
%   paragenelen, obsvstn, obsvdat, fitnessmaxlimit, allowquitfitmax, ...
%   allowquitfitavemaxratio)
%
% This is a standard genetic algorithm inversion program for the problem
% obsvdat = fwdfun(msltn, obsvstn) in the range restrain pararange of
% model parameters.
%
% Written and recommended by Tche L. at USTC, 2015/12.
%
...
for i = 1:1:popunum
  modelpopuinit(:, i) = DeCodeGA(pararange(:, 1), parasmplintv, ...
    paragenelen, modelpopuinitcode(i, :));
end
